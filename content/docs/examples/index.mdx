...
async function batchCalculate(incomes: number[]) {
  ...
  const response = await fetch(
    "https://cpf-contribution-estimator.vercel.app/api/cpf/calculate/batch",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function getAgeGroupRates(age: number) {
  const response = await fetch(
    `https://cpf-contribution-estimator.vercel.app/api/cpf/age-group/find?age=${age}`
  );
...
def project_cpf(income: float, age: int, years: int) -> dict:
    response = requests.post(
        "https://cpf-contribution-estimator.vercel.app/api/cpf/projection",
...
def compare_investments(principal: float, years: int, scenarios: list) -> dict:
    response = requests.post(
        "https://cpf-contribution-estimator.vercel.app/api/cpf/investment-comparison",
        json={
            "principal": principal,
            "years": years,
            "scenarios": scenarios
