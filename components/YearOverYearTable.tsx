        ...
        <tbody>
          {years.map((year, index) => {
            ...
            const previousResults =
              index > 0 ? distributionResults[years[index - 1]] : null;

            return (
              <tr
                key={year}
                className={year === selectedYear ? "bg-blue-50" : ""}
              >
                ...
                <td className="border border-gray-300 p-2 text-right">
                  {yearResults.employeeContribution
                    ? formatCurrency(yearResults.employeeContribution)
                    : "N/A"}
                  {previousResults && (
                    <div className="text-xs text-gray-500">
                      {yearResults.employeeContribution >
                      previousResults.employeeContribution
                ...
                <td className="border border-gray-300 p-2 text-right">
                  {yearResults.employerContribution
                    ? formatCurrency(yearResults.employerContribution)
                    : "N/A"}
                  {previousResults && (
                    <div className="text-xs text-gray-500">
                      {yearResults.employerContribution >
                      previousResults.employerContribution
                ...
                <td className="border border-gray-300 p-2 text-right">
                  {yearResults.totalContribution
                    ? formatCurrency(yearResults.totalContribution)
                    : "N/A"}
                  {previousResults && (
                    <div className="text-xs text-gray-500">
                      {yearResults.totalContribution >
                      previousResults.totalContribution
                ...
                <td className="border border-gray-300 p-2 text-right">
                  {yearResults.takeHomePay
                    ? formatCurrency(yearResults.takeHomePay)
                    : "N/A"}
                  {previousResults && (
                    <div className="text-xs text-gray-500">
