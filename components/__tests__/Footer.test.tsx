...
describe("Footer", () => {
  ...
  it("renders resources section with external links", () => {
    ...
    expect(cpfLink.closest("a")?.getAttribute("href")).toBe("https://www.cpf.gov.sg");
    expect(cpfLink.closest("a")?.getAttribute("target")).toBe("_blank");
    expect(cpfLink.closest("a")?.getAttribute("rel")).toBe("noopener noreferrer");
    
    const momLink = screen.getByText("Ministry of Manpower");
    expect(momLink).toBeTruthy();
    expect(momLink.closest("a")?.getAttribute("href")).toBe("https://www.mom.gov.sg");
  });

  it("displays the current year in copyright text", () => {
    render(<Footer />);
    expect(screen.getByText(/© 2024 CPF Calculator. All rights reserved./)).toBeTruthy();
  });
